...
@router.post("/courses/{course_id}/activities", status_code=201, response_model=Activity)
async def create_activity(course_id: int, activity: ActivityCreate = Depends(), db: get_db = Depends()):
...
async def create_activity(course_id: int, activity_id: int, db: get_db = Depends()):
